...
export default function App() {
  // board state will depend on side length; initialize with default 9
  const [board, setBoard] = useState(emptyBoard(9))
  // remember last-applied settings so we can detect changes
  ...
  const applySettings = () => {
    ...
    // if only block shape changed and board size is same, keep existing entries
    if (size === appliedSideLength) {
      // update applied shape and close
      setAppliedShape(selectedShape)
      setShowSettings(false)
      return
    }
    // otherwise we need a fresh board for new size
  ...
  // compute factor pairs when sideLength changes, excluding 1×n or n×1
  ...
  // update options whenever side length changes; show errors but keep value
  const handleSideLengthChange = (e) => {
    const v = parseInt(e.target.value, 10)
    if (isNaN(v)) {
      setSideLength(e.target.value) // allow non-number temporarily
      setSideLengthError('')
      return
    }
    setSideLength(v)
